...
const DaySquare = styled.div<{ $intensity: number; $isPlaceholder: boolean; $selected: boolean }>`
  ...
  background-color: ${({ $intensity, $isPlaceholder }) => {
    ...
    if ($intensity <= 0.33) return '#064e3b'; // Low
    if ($intensity <= 0.66) return '#059669'; // Medium
    return '#10b981'; // High
  }};
...
export function HabitHeatmap({ logs, totalHabits, onSelectDate, selectedDate }: HabitHeatmapProps) {
  ...
  return (
    <HeatmapContainer>
      ...
      <Grid>
        ...
        {days.map((day, idx) => {
          ...
          if (totalHabits > 0 && count > 0) {
            intensity = count / totalHabits;
          }

          const tooltip = `${day.dateStr}: ${count}/${totalHabits} completed`;
      ...
      <Legend>
        ...
        <LegendSquare color="#064e3b" />
        <LegendSquare color="#059669" />
